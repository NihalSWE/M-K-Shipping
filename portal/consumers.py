...
class SeatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.trip_id = self.scope["url_route"]["kwargs"]["trip_id"]
        self.group_name = f"seats_{self.trip_id}"
    ...
    async def seat_event(self, event):
        # ✅ FIX: Added start_order and end_order to the JSON payload
        await self.send(text_data=json.dumps({
            "action": event.get("action"),
            "seat_id": event.get("seat_id"),
            "holder_id": event.get("holder_id"), 
            "expires_at": event.get("expires_at"),
            "start_order": event.get("start_order"),
            "end_order": event.get("end_order"),
